...
game.import('character',function(lib,game,ui,get,ai,_status){
	return {
		...
		character:{
			...
			//shen_sunquan:['male','shen',4,['junkyuheng','junkdili'],['wu']],//神孙权就是答辩
			...
			shen_zhaoyun:['male','shen',2,['xinjuejing','relonghun','changshanyin'],['shu']],
		...
		skill:{
			...
			ol_shenfen:{
				...
				content:function(){
					...
					"step 6"
					player.turnOver();//最后再翻面
					"step 7"
					player.draw(Math.min(2*(player.countMark('baonu')),6));
					if(player.countMark('baonu')<=4){
						player.removeMark('baonu',player.countMark('baonu'));
					}
					else {player.removeMark('baonu',4)}
				},
				ai:{
					combo:'baonu',
					order:10,
			...
			renjie2:{
				...
				content:function(){
					if(trigger.getl(player).cards2.length<=2){
						player.addMark('renjie',trigger.getl(player).cards2.length);
					}
					else player.addMark('renjie',2);
				}
			},
			renjie3:{
				audio:2,
				...
				content:function(){
					if(player.isDamaged()){player.addMark('renjie',1);}
				}
			},
			sbaiyin:{
				skillAnimation:'epic',
				...
				content:function(){
					...
					player.addSkill('jilue_jizhi');
					player.removeMark('renjie',2);
					player.addSkill('jilue_huoji2');
			...
			jilue:{
				unique:true,
				group:['jilue_guicai','jilue_fangzhu','jilue_wansha','jilue_zhiheng','jilue_huoji','jilue_change']
			},
			jilue_guicai:{
				audio:1,
			...
			jilue_huoji:{
				...
				viewAsFilter:function(player){
					if (!player.countCards('hs', { color: 'red' }) || !player.hasMark('renjie')) return false;
				},
				position:'hs',
				prompt:'将一张红色牌当火攻使用',
				check:function(card){
			...
			jilue_huoji2:{
				...
				usable:1,
				filter:function(event,player){
					return (event.card.name=='huogong')&&event.targets.length==1&&player.hasMark('renjie');
				},
				content:function(){
					player.removeMark('renjie',1);
				}
			...
			xinlonghun:{
				...
				hiddenCard:function(player,name){
					if(name=='wuxie'&&_status.connectMode&&player.countCards('hs')>0) return true;
					if(name=='wuxie') return player.countCards('hs',{suit:'spade'})>0;true
			...
			changshanyin:{
				trigger:{player:'phaseZhunbeiBegin'},//开局可以发动了
				charlotte:true,
				filter: function(event, player){
					return player.maxHp > 2 && game.roundNumber<=1;
				},
				content:function(){
					player.loseMaxHp(); // 减少一点体力上限
					player.draw(player.maxHp-1); // 摸体力上限的牌
					var card=get.cardPile('qinggang','field');
					if(card){
						player.gain(card,'gain2','log');
					}//并获得青钢剑
					player.addSkill('changshanyin1')//手牌上限加1
				},
			},
			changshanyin1:{//手牌上限加1
				mod:{
					maxHandcard:function(player,num){
						return num + 1;
					}
		...
		translate:{
			...
			changshanyin:'锋拙',
			changshanyin_info:'地主专属技，游戏开始时若你的体力上限大于2你可以减少一点体力上限并摸等同体力上限的牌然后获得青钢剑，并令你本局游戏的手牌上限+1',
			wushen:'武神',
			...
			renjie:'忍戒',
			renjie2:'忍戒',
			renjie_info:'锁定技，当你受到1点伤害后，你获得一枚“忍”标记；锁定技，出牌阶段结束后若你已受伤，你获得一枚“忍”标记；锁定技，当你于弃牌阶段内弃置牌后，你获得等同于失去的牌数量的“忍”标记（至多为2）。',
			sbaiyin:'拜印',
			sbaiyin_info:'觉醒技，准备阶段开始时，若你的“忍”标记数不小于4，你减1点体力上限，然后获得〖极略〗并失去两枚忍获得〖集智〗',
			jilue:'极略',
			jilue_info:'当一名角色的判定牌生效前，你可以弃1枚“忍”标记并发动〖鬼才〗；每当你受到伤害后，你可以弃1枚“忍”标记并发动〖放逐〗；出牌阶段限一次，你可以弃2枚“忍”标记并发动〖制衡〗；出牌阶段，你可以弃1枚“忍”标记并获得〖完杀〗直到回合结束；出牌阶段,你可以弃1枚“忍”标记并发动〖火计〗且当你使用火攻时你须弃置1枚“忍”标记。',
			jilue_guicai:'鬼才',
			jilue_fangzhu:'放逐',
			jilue_wansha:'完杀',
			jilue_zhiheng:'制衡',
			...
			jilue_change:'极谋',
			lianpo:'连破',
			lianpo_info:'一名角色的回合结束时，若你本回合内杀死过角色，则你可以进行一个额外的回合并失去一枚忍标记（若有）。',
			guixin:'归心',
			qinyin:'琴音',
			yeyan:'业炎',
			...
			ol_shenfen:'神愤',
			ol_shenfen_info:'出牌阶段限一次，你可以弃置6枚“暴怒”标记并选择所有其他角色，对这些角色各造成1点伤害。然后这些角色先各弃置其装备区里的牌，再各弃置四张手牌。最后你将你的武将牌翻面然后弃置剩余标记（至多4个）并摸等同2倍剩余标记数的牌且至多为6。',
			"new_wuhun":"武魂",
