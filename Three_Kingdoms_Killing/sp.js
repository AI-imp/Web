...
game.import('character',function(lib,game,ui,get,ai,_status){
	return {
		...
		character:{
			...
			lingju:['female','qun',3,['jiwushuang','jibiyue','jieyuan','fenxin']],
			...
			hanba:['female','qun','3/4',['fentian','zhiri']],
		...
		skill:{
			...
			refuhan:{
				...
				content:function(){
					...
					'step 4'
					if(player.maxHp>=4)player.loseMaxHp(2);
				},
			},
			refanghun:{
				mod:{
				...
				content:function(){
					...
					if(result.bool){
						...
						if(player.countCards('he',{color:'red'})>0){
							player.chooseToDiscard(true,'he',{color:'red'});
							player.draw(2);
						}//摸两张牌并弃置一张红色牌
					}
				},
			...
			zhiri:{
				...
				content:function(){
					player.loseMaxHp();
					if(player.hp<player.maxHp-1){player.recover(player.maxHp-player.hp-2);}
			...
			xintan:{
				...
				group:['xindan2'],
				filter:function(event,player){
					return player.getExpansions('fentian').length>=1;
				},
				filterTarget:true,
				prompt:'移去一张“焚”并令一名角色失去一点体力',
				content:function(){
					'step 0'
					player.chooseCardButton(1,'移去一张“焚”并令'+get.translation(target)+'失去一点体力',player.getExpansions('fentian'),true);
					'step 1'
					if(result.bool){
			...
			xindan2:{
				...
				prompt:'移去一张“焚”并令一名角色摸两张牌',
				content:function(){
					'step 0'
					player.chooseCardButton(1,'移去一张“焚”并令'+get.translation(target)+'摸两张牌',player.getExpansions('fentian'),true);
					'step 1'
					if(result.bool){
						player.loseToDiscardpile(result.links);
						target.draw(2);}
			...
			jiwushuang:{
				audio:'wushuang',
				audioname:['re_lvbu','shen_lvbu','lvlingqi'],//好像只能是吕布音效
				trigger:{player:'useCardToPlayered'},
				forced:true,
				filter:function(event,player){
				...
				subSkill:{
					more:{
						...
						filter:function(event,player){
							...
							return (event.player.hp>=player.hp||event.player.hp==event.player.maxHp||player.hasSkill('fenxin_fan'))&&player!=event.player;//加强为对方满血也能放一次
						},
						content:function(){
							'step 0'
			...
			fengpo:{
				...
				filter:function(event,player){
					...
					}).indexOf(event.getParent())==0;//第一张杀
				},
				direct:true,
				content:function(){
					'step 0'
					...
					if(result.control&&result.control!='cancel2'){
						...
						var nd=trigger.target.countCards('h',{suit:'diamond'})+trigger.target.countCards('h',{suit:'heart'});
						if(result.control=='draw_card'){
							player.draw(nd+1);//改成摸对方红牌数+1，凤魄魄对面嘛
						}
						else{
						...
						trigger2.baseDamage+=Math.ceil(nd/2);//改成加伤对方(红牌数)/2向上取整
						player.draw(1);//摸一张牌
						}
					}
				}
			...
			shenxian2:{charlotte:true},//斗地主模式甚贤遇到用牌强将会变成白板技能而且就算用也只能用两张
			qiangwu:{
				audio:2,
				enable:'phaseUse',
				usable:1,
			...
			wuji:{
				...
				content:function(){
					"step 0"
					//player.removeSkill('huxiao');//不移除虎啸改为增强虎啸
					player.gainMaxHp();
					"step 1"
					player.recover();
			...
			xueji:{
				...
				selectTarget:function(){
					var player=_status.event.player
					return [1,Math.max(1,player.getDamagedHp()+1)];//已损体力值加1地主只需保持4点血即可对两农民伤害/农民满血也可
				},
				position:'he',
				filterCard:{color:'red'},
				check:function(card){
			...
			huxiao:{
				...
				content:function(){
					...
					if(player.storage.wuji){
						player.draw(1);
					}//如果已觉醒改为火焰伤害自己摸一张而对方将不再摸牌
		...
		translate:{
			...
			refuhan:'扶汉',
			refuhan_info:'限定技，回合开始时，你可以移去所有"梅影"标记并摸等量的牌，减两点体力上限然后从X张蜀势力武将牌中选择并获得至多两个技能（限定技、觉醒技、隐匿技、使命技、主公技除外）。若此时你是体力值最低的角色，你回复1点体力（X为场上角色数，且X∈[4,+∞)）。',
			yjixi:'觊玺',
			...
			xintan:'心惔',
			xintan_info:'1.出牌阶段开始时，你可以移去一张「焚」并选择一名角色，该角色摸两张牌2.出牌阶段限一次，你可以移去一张「焚」并选择一名角色，该角色失去一点体力。',
			fentian:'焚天',
			fentian_info:'锁定技，出牌阶段结束后时若你已受伤或当你受到伤害后，你须选择一名攻击范围内的角色，将其一张牌置于你的武将牌上，称为「焚」然后若你有红色牌须摸两张牌并弃置一张红色牌。锁定技，你的攻击范围+X（X为「焚」的数量）',
			zhiri:'炙日',
			zhiri_info:'觉醒技，准备阶段开始时，若你的「焚」的数量不小于3，你减1点体力上限并回复至体力上限-1，然后获得技能〖心惔〗',
			meibu:'魅步',
			...
			jiwushuang:'继无双',
			jiwushuang_info:'锁定技，当你使用【杀】指定目标后，目标角色需要依次使用或打出两张【闪】响应。',
			jibiyue:'继闭月',
			jibiyue_info:'结束阶段，若你的手牌数小于体力上限，你可以摸一张牌。',
			jieyuan:'竭缘',
			jieyuan_more:'竭缘',
			jieyuan_less:'竭缘',
			...
			fenxin:'焚心',
			fenxin_old:'焚心',
			fenxin_info:'锁定技，一名其他角色死亡后，若其身份为：忠臣，你本局内发动〖竭缘〗减少伤害时无视体力值限制；反贼，你本局内发动〖竭缘〗增加伤害时无视体力值限制；内奸，你本局内选择发动〖竭缘〗的牌时无颜色和区域限制。然后失去继无双和继闭月。',
			...
			fengpo:'凤魄',
			fengpo_info:'每种牌名各限一次。当你于出牌阶段内使用的第一张【杀】或【决斗】指定目标后，若目标角色数为1，你可以选择一项：1.摸X+1张牌；2.令此牌的伤害值基数+X/2(向上取整)并摸一张牌。（X为其手牌中♥♦牌的数量）',
			chenqing:'陈情',
			...
			qiangwu_info:'出牌阶段，你可以进行一次判定若点数不大于8，你摸两张牌。若如此做，直到回合结束，你使用点数小于判定结果的【杀】时不受距离限制，且你使用点数大于判定结果的【杀】时无使用次数限制。',
			...
			wuji_info:'觉醒技，结束阶段开始时，若你于此回合内造成过3点或更多伤害，你加1点体力上限并回复1点体力，并修改〖虎啸〗该角色摸一张牌为你摸一张牌，然后从场上、牌堆或弃牌堆中获得【青龙偃月刀】',
			xueji_info:'出牌阶段限一次，你可以弃置一张红色牌，然后选择至多X名角色，横置这些角色并对其中一名角色造成1点火焰伤害。（X为你已损失的体力值+1且至少为1）',
