...
game.import('character',function(lib,game,ui,get,ai,_status){
	return {
		...
		connect:true,
		character:{
			sb_zhugeliang:["male","shu",3,["sb_huoji","sb_kanpo"],[]],
			sb_caopi:['male','wei',3,['sbxingshang','sbfangzhu','sbsongwei','sbxingshang_die']],
		...
		characterSort:{
			sb:{
				...
				sb_shi:['sb_xuhuang','sb_machao','sb_fazheng','sb_chengong','sb_diaochan','sb_pangtong','sb_caopi','sb_zhugeliang'],
				sb_tong:['liucheng','sp_yangwan','sb_xiahoushi','sb_zhangfei','sb_zhaoyun','sb_sunce','sb_zhurong','sb_xiaoqiao'],
				sb_yu:['sb_yujin','sb_lvmeng','sb_huangzhong','sb_huanggai','sb_zhouyu','sb_caoren','sb_ganning','sb_luzhi'],
				sb_neng:['sb_huaxiong','sb_sunshangxiang','sb_jiangwei','sb_yuanshao','sb_menghuo','sb_zhanghe','sb_guanyu'],
			}
		},
		skill:{
			sb_huoji: {
				...
				subSkill: {
					achieve: {
						...
						content: function () {
							...
							player.removeSkill('sb_kanpo');
							player.addSkill('sb_guanxing');
							player.addSkill('sb_kongcheng');
							player.node.avatar.setBackground('sb_old_zhugeliang', 'character');
						},
						sub: true,
						shimingji: true,
					},
			...
			sb_kanpo: {
				...
				content: function () {
					...
					if (result.bool) {
						...
						if (player.storage.sb_kanpo.length <Math.ceil(game.players.length/2)) event.goto(0);//3个人为2，8个人为4一半向上取整
						// if (player.storage.sb_kanpo.length < 3) event.goto(0);看破2
					}
					else {
						event.finish();
					}
			...
			sbxingshang_die:{
				...
				locked:false,
				content:function(){
					player.addMark('sbxingshang');
					player.addMark('sbxingshang');//增加两个标记
				},
			},
			sbxingshang:{
				audio:2,
				...
				filter:function(event,player){
					return !player.hasHistory('useSkill',function(evt){
						return evt.skill=='sbxingshang';//未使用过
					});
				},
				forced:true,
				locked:false,
				...
				subSkill:{
					use:{
						...
						usable:2,
						filter:function(event,player){
							return player.countMark('sbxingshang')>0;
						},
						filterTarget:1,
						content:function(){
							'step 0'
							...
							for(var i=1;i<=player.countMark('sbxingshang');i++){
								...
								switch(i){
									case 1:
										list2.push('1枚：令'+get.translation(target)+'复原武将牌');
										list.push(list2);
										break;
									case 2:
										list2.push('2枚：令'+get.translation(target)+'摸'+Math.min((Math.max(game.dead.length,2)),5)+'张牌');
										list.push(list2);
										break;
									case 3:
										if(target.hp<=9) list2.push('3：令'+get.translation(target)+'回复1点体力并增加1点体力上限');
										else list2.push('3枚：无事发生');
										list.push(list2);
										break;
									case 4:
										if(target==player&&game.dead.length>0&&game.hasPlayer2(function(current){
											...
											list2.push('4枚：追思一名已阵亡的角色');
										}
										else list2.push('4枚：无事发生');
										list.push(list2);
										break;
								}
							}
							...
							if(result.bool){
								switch(result.links[0]){
									...
									case 3:
										...
										break;
									case 4:
										player.removeMark('sbxingshang',4);
										break;
								}
								if(result.links[0]!=4||target!=player) event.finish();
							}
							else event.finish();
							'step 2'
							var list=[];
			...
			sbfangzhu:{
				...
				usable:1,
				filter:function(event,player){
					return player.hasSkill('sbxingshang')&&player.countMark('sbxingshang')>=2;
				},
				...
				content:function(){
					...
					for(var i=1;i<=player.countMark('sbxingshang');i++){
						...
						switch(i){
							case 2:
								list2.push('2枚：直到'+get.translation(target)+'下回合结束，其武将技能失效或不可响应另一名角色使用的牌');
								list.push(list2);
								break;
							case 3:
								list2.push('3枚：'+get.translation(target)+'翻面，或你指定一种类型的牌，直到'+get.translation(target)+'下回合结束，其不能使用你所指定类型外的手牌');
								list.push(list2);
								break;
						}
					}
					...
					'step 2'
					player.chooseControl(['技能失效','不可响应','仅可使用锦囊牌']).set('prompt','选择令'+get.translation(target));
					'step 3'
					...
					if(result.control=='仅可使用锦囊牌'){
						player.popup('锦囊牌');
						target.addTempSkill('sbfangzhu_3',{player:'phaseEnd'});
            			target.storage.sbfangzhu_3.add(result.control);
					...
					event.finish();
					'step 5'
					player.chooseControl(['翻面','无法用牌']).set('prompt','选择令'+get.translation(target));
					'step 6'
					if(result.control=='翻面'){
						...
						player.chooseControl('basic','trick','equip',function(){
							...
							return list[0];
						}).set('prompt',get.prompt2('sbfangzhu',target));
					}
					'step 7'
					...
					3:{
						...
						mod:{
							cardEnabled:function(card,player){
								if(get.type2(card)==player.storage.sbfangzhu_3) return true;
								return false;
							},
						},
					},
			...
			sbsongwei:{
				...
				content:function(){
					...
					player.addMark('sbxingshang',num);
				},
				...
				subSkill:{
					shabi:{
						...
						content:function(){
        					...
        					player.chooseTarget('武圣：请选择一名非主公角色','本阶段你可以对其使用至多三张【杀】且无距离限制',false,(card,player,target)=>{
            				return target!=player;
        					}).set('ai',target=>{
		...
		translate:{
			...
			sb_caocao:'谋曹操',
			sbjianxiong:'奸雄',
			sbjianxiong_info:'①游戏开始时，你可获得至多2枚“治世”标记。②当你受到伤害后，你可获得伤害牌，并摸2-X张牌（X为“治世”数且至少为1），然后你可弃1枚“治世”。',
			sbqingzheng:'清正',
			...
			sb_luzhi:'谋卢植',
			sbzhenliang:'贞良',
			sbzhenliang_info:'转换技，阳：出牌阶段限一次，你可以选择一名攻击范围内的其他角色并弃置X张与“任”颜色相同的牌对其造成1点伤害（X为你与其体力值之差且至少为1)；阴：你的回合外，当你使用或打出的牌结算结束后，若此牌与“任”类别相同，则你可令一名角色摸一张牌。',
			...
			sbqicai:'奇才',
			sbqicai_backup:'奇才',
			sbqicai_info:'出牌阶段限一次，你可以将你手牌中的/弃牌堆的防具牌置入一名其他角色的装备栏（每种牌名限一次）。若你以此法将防具牌置入目标角色的装备区，则其接下来获得的三张锦囊牌须交给你。',
			sb_guanyu:'谋关羽',
			sbwusheng:'武圣',
			sbwusheng_info:'你可以将一张手牌当任意一种【杀】使用或打出。出牌阶段开始时，你可以选择一名非主公角色(斗地主模式可选择地主)，本阶段：你可以对其使用至多三张【杀】且无距离限制；当你使用【杀】指定其为目标后，你摸一张牌。',
			sbyijue:'义绝',
			sbyijue_info:'锁定技，每名角色限一次，当你于回合内对其他角色造成致命伤害时，防止之，然后本回合当你使用牌指定其为目标时，取消之。 ',
			sb_caopi:'谋曹丕',
			sbxingshang:'行殇',
			sbxingshang_info:'当一名角色死亡时或受到伤害后(每回合限一次)，你获得两个“颂”标记。出牌阶段限两次，你选择一名角色，移去任意数量的“颂”令其执行对应的一个选项:<li>1枚，复原武将牌；<li>2枚，摸X张牌(X为本局已死亡角色数，至少为2且至多为5);<li>3枚，若选择的角色体力值不大于9，回复1点体力并增加1点体力上限，然后随机恢复一个被废除的装备栏；<li>4枚，若选择的角色为你，追思一名未被追思过的已阵亡角色的武将牌上的技能。',
			sbfangzhu:'放逐',
			sbfangzhu_info:'出牌阶段限一次，若你有“行殇”，你可以选择一名其他角色，移去任意数量的“颂”令其执行对应的一个选项:<li>2枚，直到其下回合结束，其武将技能失效或不可响应另一名角色使用的牌或仅可使用锦囊牌；<li>3枚，其翻面，或你指定一种类型的牌，直到其下回合结束，其不能使用你所指定类型外的手牌。',
			sbsongwei:'颂威',
			sbsongwei_info:'主公技，出牌阶段开始时，若你有“行殇”，你获得X枚“颂”标记(X为本局游戏其他魏势力角色数)。每局游戏限一次，你可于出牌阶段令一名其他魏势力角色失去其武将牌上的所有技能。',
			sb_zhugeliang:"谋诸葛亮",
			sb_huoji: "火计",
			...
			sb_kanpo: "看破",
			sb_kanpo_info: "每轮开始时，你清除此技能记录的牌名，然后你可以记录任意个与上次记录均不相同的牌名（可同名且每局至多为存活人数一半向上取整）。当其他角色使用你记录牌名的牌时，你可以移除一个对应牌名的记录，令此牌无效并摸一张牌。",
			sb_guanxing: "观星",
