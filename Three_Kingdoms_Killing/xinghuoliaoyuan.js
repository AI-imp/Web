...
game.import('character',function(lib,game,ui,get,ai,_status){
	return {
		...
		skill:{
			...
			xinfu_jijun:{
				...
				callback:function(){
					...
					if(36 % card.number == 0){player.draw(1);}//在回调函数内根据判定点数摸牌
				},
				content:function (){
		...
		translate:{
			...
			"qinguo_lose_info":"",
			"xinfu_jijun":"集军",
			"xinfu_jijun_info":"当你于回合内使用非装备牌或武器牌指定目标后，若你是此牌的目标，你可以进行一次判定。然后，你将判定牌置于自己的武将牌上，称之为「方」且当其点数为36的约数时你摸1张牌。",
			"xinfu_fangtong":"方统",
			...
			"xinfu_zengdao2_info":"",
			"xinfu_guanwei":"观微",
			"xinfu_guanwei_info":"每回合限一次。一名角色的出牌阶段结束时，若其于出牌阶段内使用过两张及以上的牌且花色均相同，则你可以弃置一张牌，令其摸两张牌并进行一个额外的出牌阶段。",
			"xinfu_gongqing":"公清",
