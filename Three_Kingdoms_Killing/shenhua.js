					...
					effect:{
						target:function(card,player,target){
							if(get.tag(card,'damage')){
								...
								if(target.hp<=1) return;//伤害小于1就卖血
								if(!target.hasFriend()) return;
								var hastarget=false;
								var turnfriend=false;//翻队友
								var players=game.filterPlayer();
								for(var i=0;i<players.length;i++){//判断翻场上哪个人
									if(get.attitude(target,players[i])<0){//面对敌人的伤害
										for(var j=0;j<players.length;j++){
											if(get.attitude(target,players[i])>0&&playes[i].hp<=1){//先看下有无残血队友
												hastarget=true;
												turnfriend=true;//如果队友快死了而且是地主打的先翻队友-拒绝资敌
											}}
											if(!players[i].isTurnedOver()){
											hastarget=true;//敌人未翻先翻敌人,包括了以下情形如果队友快死了但是是队友打的-神赵云就反面地主
											}
								}
									if(get.attitude(target,players[i])>0&&players[i].isTurnedOver()){
										hastarget=true;
										turnfriend=true;//帮队友翻回来
									}
								}
								if(get.attitude(player,target)>0&&!hastarget) return;//队友的伤害而且没有目标就不放逐
								if(turnfriend||target.hp==target.maxHp) return [0.5,1];//如果要翻队友就翻队友
								if(target.hp>1) return [1,0.5];//伤害大于1的应对方式
							}
						}
					}
				}
